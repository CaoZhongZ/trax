...
class SequenceDataEnv(object):
  ...
  def _on_input(self):
    ...
    sequence_id, _ = self._cur_position
    return sequence_id % 2 == 0

  @property
  def observation(self):
    sequence_id, token_id = self._cur_position
    if sequence_id >= len(self._cur_sequence):
      obs = self._eos
    elif self._on_input:
      obs = self._cur_sequence[sequence_id][token_id]
    else:
  ...
  def step(self, action):
    ...
    sequence_id, token_id = self._cur_position
    if sequence_id >= len(self._cur_sequence):
      return np.array(int(self._eos), dtype=np.int32), 0.0, True, None

    if self._on_input:
      self._response = []
      if token_id + 1 < len(self._cur_sequence[sequence_id]):
        self._cur_position = (sequence_id, token_id + 1)
        done = False
      else:
        self._cur_position = (sequence_id + 1, 0)
        done = sequence_id + 1 >= len(self._cur_sequence)
      ...
      if action == self._eos or len(self._response) > self._max_length:
        self._cur_position = (sequence_id + 1, 0)
        reward = self._metric(
            self._response[:-1], self._cur_sequence[sequence_id])
        done = sequence_id + 1 >= len(self._cur_sequence)
      else:
        reward = 0.0
        done = False
...
def _accuracy(seq1, seq2):
  ...
  return np.sum(np.equal(seq1, seq2)) / max_length
